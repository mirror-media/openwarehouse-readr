...
class ImageAdapter extends GCSAdapter {
    ...
    sync_save(stream, id, origFilename) {
        ...
        this.saveOriginalSizedImage(id, ext, stream, url)
        this.saveVariusSizeImage(id, ext, stream, url)
    ...
    saveOriginalSizedImage(id, ext, stream, url) {
        const gcsUploadPath = `${this.gcsDir}${id}/${id}.${ext}`
    ...
    createUrlToUrlObject(key, url, id, resized_filename) {
        switch (key) {
            case 'tiny':
                url.urlTinySized = `${imgUrlBase}${this.gcsDir}${id}/${resized_filename}`
            case 'mobile':
                url.urlMobileSized = `${imgUrlBase}${this.gcsDir}${id}/${resized_filename}`
            case 'tablet':
                url.urlTabletSized = `${imgUrlBase}${this.gcsDir}${id}/${resized_filename}`
            case 'desktop':
                url.urlDesktopSized = `${imgUrlBase}${this.gcsDir}${id}/${resized_filename}`
        }
    }

    saveGCS(resized_filename, stream, key, id) {
        const gcsUploadPath = `${this.gcsDir}${id}/${resized_filename}`
    ...
    async delete(id, originalFilename) {
        ...
        const gcsOriginalImgDir = `${this.gcsDir}${id}/${id}.${ext}`
        imageList.push(gcsOriginalImgDir)

        for (const key in resizeTarget) {
            const imageDir = `${this.gcsDir}${id}/${id}-${key}.${ext}`
