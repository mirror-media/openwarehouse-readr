...
class ImageAdapter extends GCSAdapter {
    ...
    async save({ stream, filename, mimetype, encoding, id }) {
        ...
        try {
            for (const key in resizeTarget) {
                ...
                switch (key) {
                    case 'tiny':
                        url.urlTinySized = `${storage.imgUrlBase}${this.gcsDir}${resized_filename}`
                    case 'mobile':
                        url.urlMobileSized = `${storage.imgUrlBase}${this.gcsDir}${resized_filename}`
                    case 'tablet':
                        url.urlTabletSized = `${storage.imgUrlBase}${this.gcsDir}${resized_filename}`
                    case 'desktop':
                        url.urlDesktopSized = `${storage.imgUrlBase}${this.gcsDir}${resized_filename}`
                }
    ...
    saveOriginalSizedImage(nowtime, id, ext, stream, url) {
        ...
        url.urlOriginal = `${storage.imgUrlBase}${this.gcsDir}${nowtime}-${id}.${ext}`
    }
