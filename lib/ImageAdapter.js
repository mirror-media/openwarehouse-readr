...
class ImageAdapter extends GCSAdapter {
    ...
    saveGCS(resized_filename, stream, key) {
        ...
        sharp(stream).metadata().then(function (metadata) {
                console.log(metadata);
            }, function (err) {
                console.error(err.stack);
            }
        );
        sharp(stream).pipe(im().set('density', 72).resize(`${resizeTarget[key].width}x${resizeTarget[key].height}`).quality(85)).pipe(write) //resize and upload
    }
