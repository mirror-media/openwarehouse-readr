...
const CurrentListStore = new Store({
    ...
    getAvailableFilters() {
        return _list.columns.filter(
            (col) => col.field && col.field.hasFilterMethod
        )
    },
    getActiveFilters() {
    ...
    moveItem(prevIndex, newIndex, options) {
        ...
        if (options.manageMode) {
            ...
            _items.results.splice(
                newIndex,
                0,
                _items.results.splice(prevIndex, 1)[0]
            )
        } else {
            _items.results.splice(
                newIndex,
...
history.listen(function(location) {
    _location = location
    active.columns = _list.expandColumns(
        location.query.columns || _list.defaultColumns
    )
