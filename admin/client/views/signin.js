...
var SigninView = React.createClass({
    ...
    handleSubmit(e) {
        ...
        if (!this.state.securitycode) {
            return this.displayError(
                'Please enter the security code on your mobile to sign in.'
            )
        }
        SessionStore.signin(
            {
                ...
                if (err) {
                    ...
                    switch (error.clientMessageType) {
                        case 'SECURITY_CODE_NOT_ENABLED':
                            this.displayError(
                                'The security code of your account is not enabled yet. Please contact the system manager.'
                            )
                            break
                        case 'INVALID_SECURITY_CODE':
                            this.displayError(
                                'The security code you entered is invalid. Please try it again.'
                            )
                            break
                        ...
                        default:
                            this.displayError(
                                'The password and security code you entered are not valid.'
                            )
                            break
                    }
                    ...
                    top.location.href = this.props.from
                        ? Keystone.adminPath + this.props.from
                        : Keystone.adminPath
                }
            }
        )
    ...
    renderUserInfo() {
        ...
        let openKeystoneButton = this.props.userCanAccessKeystone ? (
            <Button href={Keystone.adminPath} type="primary">
                Open Keystone
            </Button>
        ) : null
        return (
    ...
    renderAlert() {
        if (this.state.isInvalid) {
            ...
            return (
                <div key="fake" className="Alert Alert--placeholder">
                    &nbsp;
                </div>
            )
            /* eslint-enable */
