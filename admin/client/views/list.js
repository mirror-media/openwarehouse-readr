...
const ListView = React.createClass({
    ...
    getStateFromStore() {
        ...
        state.showBlankState =
            state.ready &&
            !state.loading &&
            !state.items.results.length &&
            !state.search &&
        ...
        return (
            <InputGroup.Section className="ListHeader__create">
                <Button {...props} title={'Create ' + this.state.list.singular}>
                    ...
                    <span className="ListHeader__create__label">Create</span>
    ...
    renderManagement() {
        ...
        let actionDeleteButton = !list.nodelete ? (
            <InputGroup.Section>
                <Button onClick={this.massDelete} disabled={!checkedItemCount}>
                    Delete
                </Button>
            </InputGroup.Section>
        ) : null
        ...
        let selectAllButton =
            items.count > pageSize ? (
                <InputGroup.Section>
                    <Button
                        onClick={() => this.handleManagementSelect('all')}
                        title="Select all rows (including those not visible)"
                    >
                        All{' '}
                        <small style={buttonNoteStyles}>({items.count})</small>
                    </Button>
                </InputGroup.Section>
            ) : null
            ...
            <InputGroup.Section>
                <InputGroup contiguous>
                    ...
                    <InputGroup.Section>
                        <Button
                            onClick={() =>
                                this.handleManagementSelect('visible')
                            }
                            title="Select all rows"
                        >
                            {items.count > pageSize ? 'Page' : 'All'}{' '}
                            <small style={buttonNoteStyles}>
                    ...
                    <InputGroup.Section>
                        <Button
                            onClick={() => this.handleManagementSelect('none')}
        ...
        return (
            <InputGroup style={{ float: 'left', marginRight: '.75em' }}>
                <InputGroup.Section>
                    <Button
                        isActive={manageMode}
                        onClick={() => this.toggleManageMode(!manageMode)}
                    >
                        Manage
                    </Button>
                </InputGroup.Section>
        ...
        return (
            <Container>
                ...
                <BlankState style={{ marginTop: 40 }}>
                    <BlankState.Heading>
                        No {this.state.list.plural.toLowerCase()} found&hellip;
                    </BlankState.Heading>
    ...
    renderNoSearchResults() {
        ...
        if (this.state.filters.length) {
            matching +=
                (matching ? ' and ' : '') +
                plural(this.state.filters.length, '* filter', '* filters')
        }
        ...
        return (
            <BlankState style={{ marginTop: 20, marginBottom: 20 }}>
                ...
                />
                <BlankState.Heading>
                    No {this.state.list.plural.toLowerCase()}
                    {matching}
                </BlankState.Heading>
            </BlankState>
        )
